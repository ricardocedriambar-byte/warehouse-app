...
// Streams a file's raw bytes straight from Drive (used for the PDFs found
// by getResources) — returns the raw fetch Response so the caller can
// forward its status/body without buffering assumptions baked in here.
// This is what lets the frontend render PDFs itself (pdf.js) instead of
// embedding Drive's own /preview page, which always draws its own
// toolbar with no way to turn it off.
async function fetchFileMedia(fileId) {
  const accessToken = await getAuthToken();
  return fetch(
