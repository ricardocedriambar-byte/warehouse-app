...
// Fetches a file's raw bytes from Drive (used for the PDFs found by
// getResources) — returns the raw fetch Response so the caller can
// forward its status/headers/body without buffering assumptions baked in
// here. This is what lets the frontend render PDFs itself (pdf.js)
// instead of embedding Drive's own /preview page, which always draws its
// own toolbar with no way to turn it off.
//
// `range`, when given the client's incoming Range header verbatim, is
// forwarded straight to Drive's media endpoint (which honors it, same as
// any other HTTP file download). This is what lets pdf.js fetch only the
// handful of byte ranges it actually needs — the header/xref table, then
// just the page(s) being looked at — instead of the whole file having to
// land before it can show anything. Without this, "open a PDF" was
// bounded by the full file's download time no matter how the bytes got
// from Drive to the browser, which is why streaming vs. buffering on our
// side never actually fixed the "takes too long to load" complaint.
async function fetchFileMedia(fileId, range) {
  const accessToken = await getAuthToken();
...
module.exports = { getResources, fetchFileMedia };
