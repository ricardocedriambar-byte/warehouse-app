...
const PRICE_SYNC_STATUS_TAB = process.env.PRICE_SYNC_STATUS_TAB || 'PriceSyncStatus';

// A Sheets cell has a ~50,000 character limit, and the point of this
// status is a quick "what needs review" glance, not a full data dump — so
// each list gets capped before being persisted. Whoever actually triggered
// a sync still gets the untrimmed lists straight back in that response;
// only the saved-for-later copy is capped.
const SYNC_STATUS_LIST_CAP = 100;

function capListForStatus(list) {
  return Array.isArray(list) && list.length > SYNC_STATUS_LIST_CAP
    ? list.slice(0, SYNC_STATUS_LIST_CAP)
    : list;
}

// Persists the outcome of a price sync run (see api/sync-prices.js) so any
// later request — not just the one that triggered it — can show "last
// synced X ago, N updated, M to review" without re-running the sync.
// Vercel functions don't share memory between invocations, so this has to
// live in the Sheet like everything else here; overwrites the single
// status row each run rather than growing forever (StockLog already keeps
// a full history of every individual price change). Best-effort: a failed
// save here should never fail the sync itself, which has already
// succeeded or failed on its own by the time this is called.
async function savePriceSyncStatus(summary) {
  const trimmed = {
    ...summary,
    unmatchedInPriceList: capListForStatus(summary.unmatchedInPriceList),
    notFoundInSheet: capListForStatus(summary.notFoundInSheet),
    duplicatesInPriceList: capListForStatus(summary.duplicatesInPriceList),
    errors: capListForStatus(summary.errors)
  };
  const row = [new Date().toISOString(), JSON.stringify(trimmed)];

  try {
    await sheetsFetch(
      `/values/${encodeURIComponent(`${PRICE_SYNC_STATUS_TAB}!A2:B2`)}?valueInputOption=USER_ENTERED`,
    ...
    if (String(err.message).includes('Unable to parse range') || String(err.message).includes('400')) {
      await sheetsFetch(':batchUpdate', {
        method: 'POST',
        body: JSON.stringify({ requests: [{ addSheet: { properties: { title: PRICE_SYNC_STATUS_TAB } } }] })
      }).catch(() => {});
      await sheetsFetch(
        `/values/${encodeURIComponent(`${PRICE_SYNC_STATUS_TAB}!A1:B2`)}?valueInputOption=USER_ENTERED`,
        { method: 'PUT', body: JSON.stringify({ values: [['Timestamp', 'SummaryJSON'], row] }) }
      );
    } else {
      throw err;
    }
...
// Reads back the last saved sync status, or null if none has run yet (or
// the tab doesn't exist yet, which means the same thing here).
async function getPriceSyncStatus() {
  try {
    const data = await sheetsFetch(`/values/${encodeURIComponent(`${PRICE_SYNC_STATUS_TAB}!A2:B2`)}`);
    const row = (data.values || [])[0];
    if (!row || !row[0]) return null;
    let summary = null;
    try { summary = JSON.parse(row[1] || 'null'); } catch { summary = null; }
    return { runAt: row[0], summary };
  } catch (err) {
    return null;
  }
}
