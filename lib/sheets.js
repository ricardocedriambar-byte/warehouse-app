...
const RESOURCES_TAB = process.env.RESOURCES_TAB || 'Recursos';

// Reads the price-list / catalog links from a small dedicated tab
// (Fornecedor, Nome, URL, Tipo, Atualizado — one row per PDF). Ricardo
// edits this tab directly in Sheets: he uploads the PDF to the shared
// Drive folder, pastes its share link, and it shows up in the app
// grouped by fornecedor. If the tab doesn't exist yet, it's created
// with a header row.
async function getResources() {
  try {
    const data = await sheetsFetch(`/values/${encodeURIComponent(`${RESOURCES_TAB}!A2:E`)}`);
    const rows = data.values || [];
    return rows
      .map(r => ({
        fornecedor: (r[0] || '').trim(),
      ...
      .filter(r => r.fornecedor && r.nome && r.url);
  } catch (err) {
    if (String(err.message).includes('Unable to parse range') || String(err.message).includes('400')) {
      await sheetsFetch(':batchUpdate', {
        method: 'POST',
        body: JSON.stringify({ requests: [{ addSheet: { properties: { title: RESOURCES_TAB } } }] })
      }).catch(() => {});
      await sheetsFetch(
        `/values/${encodeURIComponent(`${RESOURCES_TAB}!A1:E1`)}:append?valueInputOption=USER_ENTERED`,
        { method: 'POST', body: JSON.stringify({ values: [['Fornecedor', 'Nome', 'URL', 'Tipo', 'Atualizado']] }) }
      );
      return [];
    }
    throw err;
