...
// Updates PRECO for many rows in a single API call. updates is an array
// of { rowNumber, preco }. Used by the price-list sync, where writing
// one row at a time would mean hundreds of separate API round-trips.
async function bulkUpdatePrices(updates) {
  if (updates.length === 0) return;

  const data = updates.map(({ rowNumber, preco }) => ({
    range: `${SHEET_TAB}!I${rowNumber}`,
    values: [[formatPtNumber(preco)]]
  }));

  // The Sheets API doesn't document a hard cap on values:batchUpdate entries,
  // but very large single requests risk timeouts — chunking keeps each
  // call small and means one failed chunk doesn't lose all prior progress.
