...
async function ensureOrdersTabExists() {
  ...
  await ensureTabExists(ORDERS_TAB, [
    ...
    'Qtd Encomendada', 'Qtd Separada', 'Preço Unitário', 'Total Linha', 'Notas Linha'
  ]);
}
...
async function createClient({ name, address, phone, email, notes }) {
  await ensureOrdersTabExists();
  const id = generateClientId();
  const row = [id, name || '', address || '', phone || '', email || '', notes || ''];
  await sheetsFetch(
    `/values/${encodeURIComponent(`${CLIENTS_TAB}!A:F`)}:append?valueInputOption=USER_ENTERED`,
    { method: 'POST', body: JSON.stringify({ values: [row] }) }
  );
  return { id, name, address, phone, email, notes };
}
...
async function getAllOrderLines() {
  await ensureOrdersTabExists();
  const data = await sheetsFetch(`/values/${encodeURIComponent(`${ORDERS_TAB}!A2:Q`)}`);
  const rows = data.values || [];
...
async function createOrder({ clientId, clientName, salesperson, orderNotes, lines }) {
  ...
  await sheetsFetch(
    `/values/${encodeURIComponent(`${ORDERS_TAB}!A:Q`)}:append?valueInputOption=USER_ENTERED`,
...
async function updateLinePicked(orderId, sku, qtyPicked) {
  ...
  rows.forEach((row, idx) => {
    if (row[ORDER_COLS.ORDER_ID] === orderId && row[ORDER_COLS.SKU] === sku) {
      targetRowIndex = idx + 2; // +2 for 1-index + header
  ...
  await sheetsFetch('/values:batchUpdate', {
    ...
    body: JSON.stringify({
      ...
      data: [
        { range: `${ORDERS_TAB}!N${targetRowIndex}`, values: [[formatPtNumber(qtyPicked)]] },
        { range: `${ORDERS_TAB}!P${targetRowIndex}`, values: [[formatPtNumber(lineTotal)]] }
      ]
    })
  });
}
