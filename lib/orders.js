...
const { sheetsFetch, parsePtNumber, formatPtNumber, appendLogEntry, findItemBySku, updateItemFields, adjustReservado } = require('./sheets');
...
function groupOrderLines(lines) {
  ...
  for (const line of lines) {
    ...
    // doorsDataRaw is only needed once, at the order level above — strip it
    // from each individual line so it doesn't get echoed back per-line.
    const { doorsDataRaw, ...lineWithoutRaw } = line;
...
async function getAllOrderLines() {
  await ensureOrdersTabExists();
  const data = await sheetsFetch(`/values/${encodeURIComponent(`${ORDERS_TAB}!A2:U`)}`);
  const rows = data.values || [];
...
async function createOrder({ clientId, clientName, salesperson, orderNotes, lines, status: requestedStatus, orderType, doorsData }) {
  ...
  const data = rows.map((row, i) => ({
    range: `${ORDERS_TAB}!A${startRow + i}:U${startRow + i}`,
    values: [row]
  }));
  ...
  if (newStatus === STATUS.CANCELLED) {
    for (const row of matchedRows) {
      ...
      try {
        ...
        const restoredStock = currentStock + piecesPicked;
        await updateItemFields(item.rowNumber, { stock: restoredStock });
