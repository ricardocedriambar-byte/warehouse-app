...
async function ensureOrdersTabExists() {
  ...
  await ensureTabExists(CLIENTS_TAB, [
    'ID', 'Nome', 'NIF', 'Morada', 'Telefone', 'Email', 'Notas'
  ]);
...
async function getAllClients() {
  ...
  const data = await sheetsFetch(`/values/${encodeURIComponent(`${CLIENTS_TAB}!A2:G`)}`);
...
async function createClient({ name, nif, address, phone, email, notes }) {
  ...
  const row = [id, name || '', nif || '', address || '', phone || '', email || '', notes || ''];
  await sheetsFetch(
    `/values/${encodeURIComponent(`${CLIENTS_TAB}!A:G`)}:append?valueInputOption=USER_ENTERED`,
    { method: 'POST', body: JSON.stringify({ values: [row] }) }
  );
  return { id, name, nif, address, phone, email, notes };
}
...
async function getAllOrderLines() {
  await ensureOrdersTabExists();
  const data = await sheetsFetch(`/values/${encodeURIComponent(`${ORDERS_TAB}!A2:R`)}`);
  const rows = data.values || [];
