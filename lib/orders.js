...
async function createOrder({ clientId, clientName, salesperson, orderNotes, lines, status: requestedStatus }) {
  await ensureOrdersTabExists();
  ...
  if (newStatus === STATUS.CANCELLED) {
    for (const row of matchedRows) {
      ...
      try {
        ...
        const restoredStock = currentStock + qtyPicked;
        ...
        await appendLogEntry({
          ...
          note: `Cancelamento encomenda ${orderId} — stock reposto`
        });
      } catch (err) {
