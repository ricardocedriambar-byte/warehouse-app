...
const COLS = {
  ...
  NOTIFY_ORDERS: 5, NOTIFY_LOWSTOCK: 6, DEFAULT_TAB: 7, AVATAR_COLOR: 8, AVATAR_PHOTO: 9
};

const VALID_ROLES = ['vendedor', 'armazém', 'admin'];

// A profile photo is stored inline as a data: URI in a single Sheets cell
// (the service account only has read-only Drive access, so there's no
// separate file-storage integration to upload it to) — the app's own
// upload flow (public/app.js's compressAvatarPhoto) already resizes and
// compresses well under this, but this is enforced here too since this
// module has no auth and is the last line of defense against a request
// that skips the client entirely.
    ...
    if (String(err.message).includes('400') || String(err.message).includes('Unable to parse')) {
      ...
      await sheetsFetch(
        `/values/${encodeURIComponent(`${USERS_TAB}!A1`)}:append?valueInputOption=USER_ENTERED`,
        {
          ...
          body: JSON.stringify({
            values: [
              ['ID', 'Nome', 'Role', 'Ativo', 'Email', 'NotificarEncomendas', 'NotificarStockBaixo', 'SeparadorInicial', 'CorAvatar', 'FotoAvatar'],
...
function rowToUser(row, rowNumber) {
  return {
    ...
    avatarPhoto: (row[COLS.AVATAR_PHOTO] || '').trim()
  };
}

async function getAllUsers({ includeInactive = false } = {}) {
  await ensureUsersTab();
  const data = await sheetsFetch(`/values/${encodeURIComponent(`${USERS_TAB}!A2:J`)}`);
...
async function createUser({ name, role, email, notifyOrders, notifyLowStock, defaultTab, avatarColor, avatarPhoto }) {
  await ensureUsersTab();
  if (avatarPhoto && avatarPhoto.length > AVATAR_PHOTO_MAX_CHARS) {
    throw new Error('Foto de perfil demasiado grande');
  }
  ...
  const row = [
    ...
    notifyOrders ? 'TRUE' : '', notifyLowStock ? 'TRUE' : '', defaultTab || '', avatarColor || '', avatarPhoto || ''
  ];
  ...
  await sheetsFetch('/values:batchUpdate', {
    ...
    body: JSON.stringify({
      ...
      data: [{ range: `${USERS_TAB}!A${rowNumber}:J${rowNumber}`, values: [row] }]
    })
  });
...
const FIELD_TO_COL = {
  ...
  defaultTab: COLS.DEFAULT_TAB, avatarColor: COLS.AVATAR_COLOR, avatarPhoto: COLS.AVATAR_PHOTO
};
...
async function updateUser(id, fields) {
  ...
  if (fields.avatarPhoto && fields.avatarPhoto.length > AVATAR_PHOTO_MAX_CHARS) {
    throw new Error('Foto de perfil demasiado grande');
  }
